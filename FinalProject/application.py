...
def trip():
    """Register trip by user."""

    # POST
    if request.method == "POST":

        # Validate form submission
        if not request.form.get("tripDate"):
            return apology("missing trip date")
        if not request.form.get("region"):
            return apology("missing region")
        if not request.form.get("time"):
            return apology("missing time")

        db.execute("INSERT INTO travel (id, tripDate, region, time, venmo, airline) VALUES(:ids, :tripDate, :region, :time, :venmo, :airline)",
                        ids=id,
